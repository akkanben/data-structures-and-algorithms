...
const countNumberOfChildren = (arr) => {
  return arr.map(element => {
    return element.children ? element.children.reduce((a) => a + 1, 0) : 0;
  }).reduce((a, b) => a + b, 0);
};
...
const calculateAverage = (arr) => {
  return arr.reduce((a, b) => a + b, 0) / arr.reduce((a) => a + 1, 0);
};


// Alternate version more inline with the Hint:
//
//const calculateAverage = (arr) => {
//  return arr.reduce((a, b, i) => {
//    a.count = i + 1;
//    a.sum = b + a.sum;
//    a.average = Math.floor(a.sum / a.count);
//    return a;
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((a, b) => isPrime(b) ? a + 1 : a + 0, 0);
};
...
const extractChildren = (arr) => {
  return arr.filter(element => element.name.match(/a/i)).reduce((a, b) => {
    if (b.children) {
