...
const getStatName = (arr, minBaseStat) => {
  return getBaseStatGreaterThan(arr, minBaseStat).map(element => element = element.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  const onlyNums = arr.filter(element => typeof element === 'number');
  return (
    onlyNums.map(element => {
