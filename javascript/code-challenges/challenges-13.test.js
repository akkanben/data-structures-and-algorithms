...
const findEvery = (arr, target) => {
  return !arr.map(element => element.includes(target) ? true : false).includes(false);
};
...
const unenrollBrook = (arr) => {
  return arr.map(element => element.filter(str => !str.includes('Brook')));
};
...
const sortByDay = (arr) => {
  return daysOfWeek.map((day) => {
    return arr.filter(element => element.includes(day));
  });
};
